...
const AUTHENTICATION_ROUTE_REGISTER = "auth/register";
...
function apiFacade() {
  ...
  const register = (person) => {
    const payload = {username: person.username, password:person.password, role: person.role}
    const options = makeOptions("POST", payload);
  
    return fetch(URL + AUTHENTICATION_ROUTE_REGISTER, options)
      .then(handleHttpErrors)
      .then((json) => {
        setToken(json.token);
      })
